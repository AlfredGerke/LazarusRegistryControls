...
begin
  ...
  RegisterComponentEditor(TRegComboBox, TRegistryControlComponentEditor);
end;

{ TCustomRegComboBox }

function TCustomRegComboBox.GetKeyByItemIndex(aItemIndex: integer): string;
var
  list: TStrings;
begin
  Result := EmptyStr;
...
function TCustomRegComboBox.GetKeyBySourceKind(aItemIndex: integer): string;
begin
...
function TCustomRegComboBox.RefreshRegistrySettings: boolean;
begin
  Result := False;
...
procedure TCustomRegComboBox.ReadWriteInfo(aRead: boolean);
var
...
function TCustomRegComboBox.GetItemsByRegistry: boolean;
var
  list: TStrings;
  index: integer;
begin
...
procedure TCustomRegComboBox.RefreshMerge(var aMessage: TLMessage);
var
...
procedure TCustomRegComboBox.ClearClientItems(var aMessage: TLMessage);
var
  group_index: cardinal;
begin
  group_index := aMessage.lParam;
...
procedure TCustomRegComboBox.RefreshWriteAdHoc(var aMessage: TLMessage);
var
  group_index: cardinal;
...
procedure TCustomRegComboBox.RefreshSync(var aMessage: TLMessage);
var
  group_index: cardinal;
...
procedure TCustomRegComboBox.RefreshSettings(var aMessage: TLMessage);
begin
  aMessage.Result := LongInt(RefreshRegistrySettings);
end;

procedure TCustomRegComboBox.RefreshData(var aMessage: TLMessage);
var
  group_index: cardinal;
begin
...
procedure TCustomRegComboBox.PostData(var aMessage: TLMessage);
var
  group_index: cardinal;
begin
  if FRegistrySettings.DoSyncData then
...
procedure TCustomRegComboBox.DeleteItem(aItemIndex: integer = -1);
var
  key: string;
begin
...
procedure TCustomRegComboBox.SetRegistrySource(aRegistrySource: TRegistrySource);
begin
...
procedure TCustomRegComboBox.AfterConstruction;
begin
  inherited AfterConstruction;
end;

function TCustomRegComboBox.ReadFromReg: boolean;
begin
  Result := False;
...
function TCustomRegComboBox.WriteToReg: boolean;
begin
  Result := False;
...
constructor TCustomRegComboBox.Create(AOwner: TComponent);
begin
...
destructor TCustomRegComboBox.Destroy;
begin
  if Assigned(FRegistrySource) then
    FRegistrySource.UnRegisterClient(self);
