...
function TCustomRegLabel.RefreshRegistrySettings: boolean;
begin
  Result := False;
...
procedure TCustomRegLabel.ReadWriteInfo(aRead: boolean);
var
...
procedure TCustomRegLabel.ShowEditDialog(var aMessage: TLMessage);
var
  do_edit: boolean;
  design_time: boolean;
begin
...
procedure TCustomRegLabel.FreeRegistrySource(var aMessage: TLMessage);
begin
  if Assigned(FRegistrySource) then
...
procedure TCustomRegLabel.RefreshWriteAdHoc(var aMessage: TLMessage);
var
...
procedure TCustomRegLabel.RefreshSync(var aMessage: TLMessage);
var
  group_index: cardinal;
...
procedure TCustomRegLabel.RefreshSettings(var aMessage: TLMessage);
begin
  aMessage.Result := LongInt(RefreshRegistrySettings);
end;

procedure TCustomRegLabel.RefreshData(var aMessage: TLMessage);
var
  group_index: cardinal;
begin
...
procedure TCustomRegLabel.PostData(var aMessage: TLMessage);
var
  group_index: cardinal;
begin
  if FRegistrySettings.DoSyncData then
...
procedure TCustomRegLabel.TextChanged;
begin
  inherited;
...
procedure TCustomRegLabel.SetName(const NewName: TComponentName);
var
...
procedure TCustomRegLabel.OnChangeSettings(Sender: TObject);
begin
  ReadFromReg;
end;

procedure TCustomRegLabel.SetRegistrySource(aRegistrySource: TRegistrySource);
begin
...
function TCustomRegLabel.GetEditDialog(aEdit: boolean;
  aAtDesignTime: boolean): boolean;
var
...
procedure TCustomRegLabel.AfterConstruction;
begin
  inherited AfterConstruction;
end;

function TCustomRegLabel.ReadFromReg: boolean;
begin
  Result := False;
...
function TCustomRegLabel.WriteToReg: boolean;
begin
  Result := False;
...
constructor TCustomRegLabel.Create(AOwner: TComponent);
begin
...
destructor TCustomRegLabel.Destroy;
begin
  if Assigned(FRegistrySource) then
    FRegistrySource.UnRegisterClient(self);
